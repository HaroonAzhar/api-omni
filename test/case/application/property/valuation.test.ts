...
describe("Valuation Flow", () => {
  ...
  it("should save data", async (done) => {
    ...
    const payload = await import("../../json/application_valuation_flow");
    ...
    for (const property of form[0].properties) {
      request(app)
        .patch(`/cases/${formId}/property/${property.id}/valuation_report`)
        ...
        .end(async (err, res) => {
          ...
          expect(
            application[0].properties[propertyIndex].valuation_report
          ).toMatchObject(payload.default.data.attributes);
          done();
        });
    }
  });
