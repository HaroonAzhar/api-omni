...
describe("OriginationChecklistCreditSafe (e2e)", () => {
  ...
  it("OriginationChecklistCreditSafe (POST GET)", async (done) => {
    ...
    return waterfall(
      [
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          ...
          const { creditSafe } = furtherDrawdown.originationChecklist;
          expect(creditSafe.primarySignature).toBeUndefined();
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/creditSafe/primarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { creditSafe } = furtherDrawdown.originationChecklist;
          expect(creditSafe.primarySignature.User).toBe(primarySignature.User);
          expect(creditSafe.primarySignature.Date).toBe(
            primarySignature.Date.toISOString()
          );
          expect(creditSafe.secondarySignature).toBeUndefined();
          cb(null, results);
        },
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/creditSafe/secondarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { creditSafe } = furtherDrawdown.originationChecklist;
          expect(creditSafe.secondarySignature.User).toBe(
            secondarySignature.User
          );
          expect(creditSafe.secondarySignature.Date).toBe(
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/creditSafe/directorsListedTheSame`
            )
            .send({ DirectorsListedTheSame: true })
            .expect(201, cb),

        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/creditSafe/ensureNoCCJ`
            )
            .send({ EnsureNoCCJ: true })
            .expect(201, cb),

        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/creditSafe/nameMatchesOfferLetter`
            )
            .send({ NameMatchesOfferLetter: true })
            .expect(201, cb),
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { creditSafe } = furtherDrawdown.originationChecklist;
          expect(creditSafe.DirectorsListedTheSame).toBe(true);
          expect(creditSafe.EnsureNoCCJ).toBe(true);
          expect(creditSafe.NameMatchesOfferLetter).toBe(true);
