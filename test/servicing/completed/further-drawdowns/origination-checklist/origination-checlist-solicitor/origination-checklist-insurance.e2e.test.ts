...
describe("OriginationChecklistInsurance (e2e)", () => {
  ...
  it("OriginationChecklistInsurance (POST GET)", async (done) => {
    ...
    return waterfall(
      [
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          ...
          const { insurance } = furtherDrawdown.originationChecklist;
          expect(insurance.primarySignature).toBeUndefined();
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/insurance/primarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { insurance } = furtherDrawdown.originationChecklist;
          expect(insurance.primarySignature.User).toBe(primarySignature.User);
          expect(insurance.primarySignature.Date).toBe(
            primarySignature.Date.toISOString()
          );
          expect(insurance.secondarySignature).toBeUndefined();
          cb(null, results);
        },
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/insurance/secondarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { insurance } = furtherDrawdown.originationChecklist;
          expect(insurance.secondarySignature.User).toBe(
            secondarySignature.User
          );
          expect(insurance.secondarySignature.Date).toBe(
