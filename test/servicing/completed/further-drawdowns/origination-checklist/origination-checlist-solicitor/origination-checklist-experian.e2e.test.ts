...
describe("OriginationChecklistExperian (e2e)", () => {
  ...
  it("OriginationChecklistExperian (POST GET)", async (done) => {
    ...
    return waterfall(
      [
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          ...
          const { experian } = furtherDrawdown.originationChecklist;
          expect(experian.primarySignature).toBeUndefined();
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/experian/primarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { experian } = furtherDrawdown.originationChecklist;
          expect(experian.primarySignature.User).toBe(primarySignature.User);
          expect(experian.primarySignature.Date).toBe(
            primarySignature.Date.toISOString()
          );
          expect(experian.secondarySignature).toBeUndefined();
          cb(null, results);
        },
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/experian/secondarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { experian } = furtherDrawdown.originationChecklist;
          expect(experian.secondarySignature.User).toBe(
            secondarySignature.User
          );
          expect(experian.secondarySignature.Date).toBe(
