...
import { AddResultDto } from "../../../../../../src/v2/modules/cases/completed/further/origination-checklist/origination-checklist-land-registry/dtos/add-result.dto";

describe("OriginationChecklistLandRegistry (e2e)", () => {
  ...
  it("OriginationChecklistLandRegistry (POST GET)", async (done) => {
    ...
    const landRegistryResult: AddResultDto = {
      FkSecurityId: 1,
      LandRegistrySearchRun: true,
      NoOtherCharges: true,
      OmniNoted: true,
    };

    const otherLandRegistryResult: AddResultDto = {
      FkSecurityId: 2,
      LandRegistrySearchRun: true,
      NoOtherCharges: true,
      OmniNoted: true,
    ...
    return waterfall(
      [
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          ...
          const { landRegistry } = furtherDrawdown.originationChecklist;
          expect(landRegistry.primarySignature).toBeUndefined();
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/landRegistry/primarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { landRegistry } = furtherDrawdown.originationChecklist;
          expect(landRegistry.primarySignature.User).toBe(
            primarySignature.User
          );
          expect(landRegistry.primarySignature.Date).toBe(
            primarySignature.Date.toISOString()
          );
          expect(landRegistry.secondarySignature).toBeUndefined();
          cb(null, results);
        },
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/landRegistry/secondarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { landRegistry } = furtherDrawdown.originationChecklist;
          expect(landRegistry.secondarySignature.User).toBe(
            secondarySignature.User
          );
          expect(landRegistry.secondarySignature.Date).toBe(
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/landRegistry/results`
            )
            .send(landRegistryResult)
            .expect(201, cb),
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { landRegistry } = furtherDrawdown.originationChecklist;
          expect(landRegistry.results.length).toBe(2);

          expect(landRegistry.results[0]).toEqual(
            expect.objectContaining(landRegistryResult)
          );
          expect(landRegistry.results[1]).toEqual(
            expect.objectContaining(otherLandRegistryResult)
          );
