...
describe("OriginationChecklistSolicitor (e2e)", () => {
  ...
  it("OriginationChecklistSolicitor (POST GET)", async (done) => {
    ...
    return waterfall(
      [
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          ...
          const { solicitor } = furtherDrawdown.originationChecklist;
          expect(solicitor.primarySignature).toBeUndefined();
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/solicitor/primarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { solicitor } = furtherDrawdown.originationChecklist;
          expect(solicitor.primarySignature.User).toBe(primarySignature.User);
          expect(solicitor.primarySignature.Date).toBe(
            primarySignature.Date.toISOString()
          );
          expect(solicitor.secondarySignature).toBeUndefined();
          cb(null, results);
        },
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/solicitor/secondarySignature`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { solicitor } = furtherDrawdown.originationChecklist;
          expect(solicitor.secondarySignature.User).toBe(
            secondarySignature.User
          );
          expect(solicitor.secondarySignature.Date).toBe(
        ...
        (results: request.Response, cb: request.CallbackHandler) =>
          request(server)
            .post(
              `/cases/${caseId}/completed/furtherDrawdowns/${furtherDrawdownId}/originationChecklist/solicitor/comments`
            )
        ...
        (results: request.Response, cb: request.CallbackHandler) => {
          const furtherDrawdown: FurtherDrawdown =
            results.body[results.body.length - 1];
          const { solicitor } = furtherDrawdown.originationChecklist;
          expect(solicitor.Comments).toBe("Comments");
          cb(null, results);
        },
        () => {
          done();
